...
<script language="javascript">
	$(function() {
		$("#b").change(function() {
			var b_ = $("#b").attr("value");
			var check_fl = 'service_fl';
			$.post("check_floor.php", {
				b: b_,
				check_fl: check_fl
			}, function(data) {
				$("#floor_sel").html(data);
	...
	<?php if ($_SESSION['user_status_session'] != '0') { ?>
		$(function() {
			$("#bt").click(function() {
				var p = $("#p").attr("value");
				var b = $("#b").attr("value");
				var floor_ = $("#floorr").attr("value");
				var room = $("#room").attr("value");
				var d = $("#d").attr("value");
				var l = $("#l").attr("value");
				if (p == '' || b == '0' || floor_ == '0' || room == '0' || d == '') {
...
<center>
	...
	function problem_sel($mysqli)
	{
		...
		while ($prob < $num_problem_sel) {
			$fetch_problem_sel  = mysqli_fetch_array($result_problem_sel);
	...
	<p><br>
	<div style="text-align:right;" class="title"><?php echo "<b>วันที่แจ้ง :</b> ";
													ConThai($ddate); ?></div>
	<form method="post">
		<table class="form">
			<tr>
			...
			<tr>
				...
				<td colspan="3"><?php echo "$fetch_prename[prename]$fetch_username[name] $fetch_username[lastname]" ?></td>
			</tr>
			<tr>
			...
			<tr>
				<th align="center" style="text-align:center;" class="form" scope="col" colspan="4">|------------------------------ ข้อมูลแจ้งซ่อม -------------------------------|</th>
			</tr>
			<tr>
				<th scope="row">ประเภทที่แจ้ง : </th>
				<td><?php problem_sel($mysqli); ?></td>
				<?php if ($_SESSION['user_status_session'] != '0') { ?>
					<th scope="row">อาคาร : </th>
					<td><?php building_sel($mysqli); ?></td>
			</tr>
			<tr>
				<th scope="row">ชั้น : </th>
				<td><label id="floor_sel">
						<font color="#FF0000">กรุณาเลือกอาคารก่อน</font>
					</label></td>
				<th scope="row">ห้อง : </th>
				<td><label id="room_sel">
						<font color="#FF0000">กรุณาเลือกชั้นก่อน</font>
					</label></td>
			</tr>
		<?php } else {
			...
			<td>
				<?php
					...
					while ($roo < $num_room_user_sel) {
						$fetch_room_user_sel  = mysqli_fetch_array($result_room_user_sel);
		...
		<tr>
			...
			<td colspan="2"><textarea rows="3" id="d"></textarea></td>
			<td align="center"><input type="button" value="ยืนยันการแจ้งซ่อม" title="ส่งเรื่องแจ้งซ่อม" id="bt"></td>
		</tr>
		</table>
	</form>
	<div id="msg_"></div>
