...
<body>
	<div class='title'>
		...
		<p align="center" class='title'><b>รายการปฏิบัติงานตามใบแจ้งซ่อมไฟฟ้า ประปา โทรศัพท์ และอื่นๆ<br>ของหน่วยอาคารสถานที่และยานพาหนะ บางเขน</b><br>
			<?php if ($month == '01') {
				echo "ประจำเดือน มกราคม 2554";
			} else if ($month == '02') {
				echo "ประจำเดือน กุมภาพันธ์ 2554";
			} else if ($month == '03') {
				echo "ประจำเดือน มีนาคม 2554";
			} else if ($month == '04') {
				echo "ประจำเดือน เมษายน 2554";
			} else if ($month == '05') {
				echo "ประจำเดือน พฤษภาคม 2554";
			} else if ($month == '06') {
				echo "ประจำเดือน มิถุนายน 2554";
			} else if ($month == '07') {
				echo "ประจำเดือน กรกฎาคม 2554";
			} else if ($month == '08') {
				echo "ประจำเดือน สิงหาคม 2554";
			} else if ($month == '09') {
				echo "ประจำเดือน กันยายน 2554";
			} else if ($month == '10') {
				echo "ประจำเดือน ตุลาคม 2554";
			} else if ($month == '11') {
				echo "ประจำเดือน พฤศจิกายน 2554";
			} else if ($month == '12') {
				echo "ประจำเดือน ธันวาคม 2554";
			} ?>
	...
	<table class="css">
		...
		} else if ($_SESSION['user_status_session'] == "1" || $_SESSION['user_status_session'] == "2") {
			$sql_page = $mysqli->query("SELECT count(date) as date FROM list_service");
		}
		...
		if ($tech_ != "0") {
			$sql .= " AND (id_technician_a='$tech_' OR id_technician_b='$tech_')";
		}
		if ($id_building_ != "0") {
			$sql .= " AND id_building='$id_building_'";
		}
		if ($job_status_ != "0") {
			$sql .= " AND job_status='$job_status_'";
		}
		...
		<?
			...
			print "<tr><td>";
			Conthai($date_temps);
			echo "</td><td>$t</td><td>$sector_temps</td><td>$problem_temps</td><td>";

			if ($job_status_temps == "รอการอนุมัติ") {
				echo "<font color='black'>รอการอนุมัติ</font>";
			} else if ($job_status_temps == "อนุมัติแล้วกำลังรอการดำเนินการ") {
				echo "<font color='blue'>อนุมัติแล้วกำลังรอการดำเนินการ</font>";
			} else if ($job_status_temps == "อยู่ในระหว่างการดำเนินการ") {
				echo "<font color='#FFA500'>อยู่ในระหว่างการดำเนินการ</font>";
			} else if ($job_status_temps == "ดำเนินการเรียบร้อย") {
				echo "<font color='green'>ดำเนินการเรียบร้อย</font>";
			} else if ($job_status_temps == "ไม่สามารถดำเนินการได้") {
				echo "<font color='#FF7F50'>ไม่สามารถดำเนินการได้</font>";
			} else if ($job_status_temps == "ไม่อนุมัติ") {
